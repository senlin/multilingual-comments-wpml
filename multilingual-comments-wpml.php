...
class Multilingual_Comments_WPML
{
    ...
    /**
     * Checks if WPML is active before activating the plugin.
     */
    ...
    /**
     * Ensures WPML is active; otherwise, deactivates the plugin.
     */
    public function activate()
    {
        if (!$this->is_wpml_active()) {
            deactivate_plugins(plugin_basename(__FILE__));
            wp_die(__('The "Multilingual Comments (WPML)" plugin requires WPML to be installed and activated.', 'multilingual-comments-wpml'));
        }
    }

    /**
     * Sorts merged comments by comment ID.
     */
    ...
    /**
     * Merges comments from all translations of a post.
     */
    public function merge_comments($comments, $post_ID)
    {
        ...
        foreach ($languages as $l) {
            if (!$l['active']) {
                ...
                if ($otherID) {
                    $othercomments = get_comments(['post_id' => $otherID, 'status' => 'approve', 'order' => 'ASC']);
    ...
    /**
     * Fixes pagination by modifying the comment query to include all translations.
     */
    public function modify_comments_query($clauses, $query)
    {
        global $wpdb, $sitepress;
        if (empty($query->query_vars['post_id'])) return $clauses;

        $post_ID = $query->query_vars['post_id'];
        $post = get_post($post_ID);
        if (!$post) return $clauses;
        ...
        foreach ($languages as $l) {
            if (!$l['active']) {
                ...
                if ($translated_id) $post_ids[] = $translated_id;
            }
        }
    ...
    /**
     * Merges comment counts across translations.
     */
        ...
        foreach ($languages as $l) {
            if (!$l['active']) {
                ...
                if ($otherID) {
                    ...
                    if ($otherpost) $count += $otherpost->comment_count;
                }
            }
        }
        return $count;
